...
iter_reduce(seqiterobject *it, PyObject *Py_UNUSED(ignored))
{
    if (it->it_seq != NULL)
        ...
        _builtResult1_tupleMember2 = PyLong_FromSsize_t(it->it_index);
        #else
        _builtResult1_tupleMember2 = PyLong_FromLong(it->it_index);
        #endif
...
calliter_reduce(calliterobject *it, PyObject *Py_UNUSED(ignored))
{
    if (it->it_callable != NULL && it->it_sentinel != NULL)
        ...
        PyObject* _objectArg = (PyObject*) it->it_callable;
        if (_objectArg) {
            Py_INCREF(_objectArg);
        ...
        PyObject* _objectArg = (PyObject*) it->it_sentinel;
        if (_objectArg) {
            Py_INCREF(_objectArg);
